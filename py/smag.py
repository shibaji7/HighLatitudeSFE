...
"""smag.py: module is dedicated to get magnetic data from CSV files."""
...
class SuperMAG(object):
    ...
    def fetch_baseline_substracted_datasets(self):
        ...
        extent = int((self.dates[1] - self.dates[0]).total_seconds())
        fname = self.base + f"supermag-{self.dates[0].day}.csv"
...
if __name__ == "__main__":
    SuperMAG.FetchSM(
        "database/", 
        [
            dt.datetime(2017,9,6,11,30),
            dt.datetime(2017,9,6,16,30),
        ]
    )
    SuperMAG.FetchSM(
        "database/", 
        [
            dt.datetime(2017,9,10,15),
            dt.datetime(2017,9,10,20),
        ]
    )