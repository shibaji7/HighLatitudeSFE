...
def sm_DateToYMDHMS(tval, yr, mo, dy, hr, mt, sc):
    ...
    datestr = pd.to_datetime(
        julday, unit="D", origin="julian"
...
def sm_keycheck_data(flagstring):
    ...
    for i in range(0, len(flags)):
        ...
        if chk == "all":
            myflags += "&mlt&mag&geo&decl&sza"
        for ikey in range(0, len(toggles)):
            if chk == toggles[ikey]:
                myflags += "&" + toggles[ikey]
...
def sm_keycheck_indices(flagstring):
    ...
    for i in range(0, len(flags)):
        ...
        if chk == "baseall":
            indices += ",".join(basekeys)
        if chk == "sunall":
            indices += ",".join(sunkeys)
        if chk == "darkall":
            indices += ",".join(darkkeys)
        if chk == "regionalall" or chk == "regall":
            indices += ",".join(regkeys)
        if chk == "plusall":
            indices += ",".join(pluskeys)
    ...
    try:
        ...
        with urllib.request.urlopen(fetchurl, cafile=cafile) as response:
            ...
            if fetch == "json":
                ...
                success = 1  # it worked
                if re.search(r"ERROR", mydata[0]):
                    success = 0  # legit return, but of an err
...
def supermag_testing(userid):
    ...
    start = [2019, 11, 15, 10, 40, 0]  # alt: start='2019-11-15T10:40'
    ...
    print(
        stationlist[0]
    )  # prints a list of stations as a string, but cannot easily access a single item because it is a pandas series
    ...
    pairsets = [
        (myeach["tval"], myeach["density"], myeach["vgse"]["X"]) for myeach in mydata2c
    ]  # same, pull out pairs, only assign e.g. x=tval, y=density
